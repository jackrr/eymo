...
pub fn texture_to_img(gpu: &mut GpuExecutor, texture: &wgpu::Texture) -> Result<RgbImage> {
    let span = span!(Level::INFO, "texture_to_img");
    let _guard = span.enter();

    let shader_code = wgpu::include_wgsl!("rgb_img.wgsl");
    let shader = gpu.load_shader("tex_to_img", shader_code);
    ...
    let compute_pipeline = gpu
        ...
        .create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            ...
            layout: None,
            module: &shader,
            entry_point: Some("texture_to_rgb_buf_u8"),
    ...
    let exec_span = span!(Level::INFO, "exec");
    let exec_guard = exec_span.enter();
    ...
    // takes 5-19ms
    let mem_map_span = span!(Level::INFO, "mem_map");
    let mem_map_guard = mem_map_span.enter();
    ...
    let res = bytemuck::cast_slice::<u8, u8>(&*buffer_data).to_vec();

    match RgbImage::from_raw(texture.width(), texture.height(), res) {
        Some(i) => Ok(i),
        None => Err(Error::msg("Failed to convert buffer to image")),
    }
}
